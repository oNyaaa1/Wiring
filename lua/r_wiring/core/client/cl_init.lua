...
net.Receive("RWiring_RemoveConnection", function()
    ...
    for i = #WiringConnections, 1, -1 do
        ...
        if v[1] == outNode or v[2] == inNode or not IsValid(v[1]) or not IsValid(v[2]) then table.remove(WiringConnections, i) end
    end
end)
...
hook.Add("PostDrawOpaqueRenderables", "RWiring_DrawCables", function()
    ...
    for i = #WiringConnections, 1, -1 do
        local v = WiringConnections[i]
        if not IsValid(v[1]) or not IsValid(v[2]) then table.remove(WiringConnections, i) end
    end
